...
public class CancelCommand(MenuStateMachine<ChessMenuState, ChessMenuStateCommand> stateMachine, ScreenInvalidator screenInvalidator)
    : ICommand
{
    ...
    public Task Execute()
    {
        _stateMachine.ExecuteCommand(ChessMenuStateCommand.Cancel);
